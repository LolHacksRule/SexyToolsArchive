...
void HTTPTransfer::GetThreadProc()
{
		...
		else 
		{
			...
			while (!mExiting)
			{
				...
				if (aLastPos == aPos)
				{
					...
					{
						int anError = WSAGetLastError();
						if ((aContentLengthLeft > 0) || (aChunkLengthLeft > 0))
							Fail(RESULT_DISCONNECTED);
						break;
					}
				}				
				...
				if (!gotHeader)
				{
					...
					char* aCheckStr = "Location: ";
					if (strncmp(aLine.c_str(), aCheckStr, strlen(aCheckStr)) == 0)
					{
						mRedirect = aLine.substr(strlen(aCheckStr));
					}

					aCheckStr = "Transfer-Encoding: ";
					if (strncmp(aLine.c_str(), aCheckStr, strlen(aCheckStr)) == 0)
					{
					...
					aCheckStr = "Content-Range: bytes ";
					int aCheckLen = strlen(aCheckStr);
					if (strncmp(aLine.c_str(), aCheckStr, aCheckLen) == 0)
					{						
						int aMinusPos = aLine.find('-', aCheckLen);
						std::string aNumString = aLine.substr(aCheckLen, aMinusPos - aCheckLen);
						mPartialStart = atoi(aNumString.c_str());
					}
				...
				{
					std::string* aContentString;
					if (mContent.size() == 0)					
						mContent.push_back(std::string());
					
					aContentString = &mContent[mContent.size()-1];
					if (aContentString->length() >= 1024*1024)
					{
						// Add a new string if it gets too big
						mContent.push_back(std::string());
						aContentString = &mContent[mContent.size()-1];
					}
					...
					if (chunked)
					{
						...
						if (aCopyLen > 0)
						{							
							*aContentString += aRecvStr.substr(aPos, aCopyLen);
	...
	{
		if (mRedirect.length() > 0)
			mResult = RESULT_HTTP_REDIRECT;
		else
			mResult = RESULT_DONE;
	}
...
void HTTPTransfer::GetHelper(const std::string& theURL)
{
	...
	mSendStr = 
		"GET " + mPath + " HTTP/1.0\r\n" 
		"User-Agent: Mozilla/4.0 (compatible; Opera 4.0)\r\n"
		"Host: " + mHost + "\r\n"
		"Connection: close\r\n" +
		"\r\n";
